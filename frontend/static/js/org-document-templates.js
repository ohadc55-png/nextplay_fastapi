...
(function () {
  ...
  function renderRows(templates) {
    ...
    $rows.replaceChildren.apply(
      ...
      templates.map(function (t) {
        var nameCell = el("td", null, [el("strong", { text: t.name })]);
        if (t.description) {
        ...
        var statusCell = el("td", null, [
          pill(t.is_active ? "פעיל" : "לא פעיל", t.is_active ? "" : "org-pill--muted"),
        ]);
        ...
        if (canManage) {
          ...
          if (t.uploaded_file_type === "PDF") {
            // Edit-fields link (PDF only — DOCX field marking deferred).
            ...
            // Send-to-recipients button (PDF + active templates only).
            if (t.is_active) {
  ...
  document.addEventListener("click", function (e) {
    var t = e.target.closest("[data-action], [data-delete-template], [data-send-template]");
    if (!t) return;
